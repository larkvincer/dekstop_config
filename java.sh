...
#Downloading package
sudo download_package "$DWN_URL" "$DWN_PACKAGE" "$DWN_FOLDER" "Cookie: oraclelicense=accept-securebackup-cookie"
#wget -c --no-check-certificate --no-cookies --header "Cookie: oraclelicense=accept-securebackup-cookie" "${BASE_URL}"

# Extract archive
sudo unpack_package "$DWN_PACKAGE" "$DWN_FOLDER"

# File for setting path
sudo echo "export JAVA_HOME=/opt/$DIR_NAME" >> "$PATH_FILE"
